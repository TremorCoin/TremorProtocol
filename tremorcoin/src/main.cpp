...
#if defined(NDEBUG)
# error "Tremorcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0d643af72bc75b51bd2449bc62b2f4e7b806b5ea2cbf21a831d5c0522b9f1314");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tremorcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tremorcoin Signed Message:\n";
...
{
    // Tremorcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 200000 blocks, which will occur approximately every 1.2 years
    nSubsidy >>= (nHeight / 200000); // Tremorcoin: 200k blocks in ~1.2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tremorcoin: 1 day
static const int64 nTargetSpacing = 3 * 60; // Tremorcoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tremorcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tremorcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdc22bee5caba554d00fac94790975a28f50ff90808428e9b75651e26c9998294");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "USA Today 23/Jan/2018 1.7-billion-year-old chunk of North America found in Australia";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 280 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04e2ce31a3a608701d2401e70c0ff07bf6fa66c485b5d0d16111b86aa1cbd44611a4853b728fff76681671d086cecf42f875c9c12949ebd6f6ceeb635ed79b58d6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516769745;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085375939;

        if (fTestNet)
        {
            block.nTime    = 1516769727;
            block.nNonce   = 386483278;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x88bdfa229caaa2ddf9b0d412bef9bb11557e52da6f374cb84ab4488508ffbede"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb1, 0xd2 }; // Tremorcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TremorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TremorcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TremorcoinMiner(CWallet *pwallet)
{
    printf("TremorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tremorcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TremorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TremorcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TremorcoinMiner, pwallet));
}
